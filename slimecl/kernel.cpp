...
return R( // ########################## begin of OpenCL C code ####################################################################
	...
	float random01(uint seed)
	{
		...
		return convert_float(randomInt(seed)) / 4294967296;
	}
		...
		int mapWidth, int mapHeight, float simDeltaTime, struct TrailSettings trailSettings)
	{
		...
		int2 s2 = (int2)(wrap(x    , mapWidth), wrap(y - 1, mapHeight));
		int2 s3 = (int2)(wrap(x + 1, mapWidth), wrap(y - 1, mapHeight));
		int2 s4 = (int2)(wrap(x - 1, mapWidth), wrap(y    , mapHeight));
		int2 s6 = (int2)(wrap(x + 1, mapWidth), wrap(y    , mapHeight));
		int2 s7 = (int2)(wrap(x - 1, mapWidth), wrap(y + 1, mapHeight));
		int2 s8 = (int2)(wrap(x    , mapWidth), wrap(y + 1, mapHeight));
	...
	kernel void updateSlimes(global float2* positions, global float2* directions, global float* trailMap,
		global float* nextTrailMap, int mapWidth, int mapHeight, float simDeltaTime, float elapsedTime,
		struct SlimeSettings slimeSettings)
	{
		...
		float rdx = random01(slimeIndex + elapsedTime) - 0.5f;
		float rdy = random01(slimeIndex + elapsedTime + 12345) - 0.5f;
		float2 randDir = (float2)(rdx, rdy);
